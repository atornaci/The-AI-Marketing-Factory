...
export async function POST(req: NextRequest) {
    try {
        ...
        const adCopies = await abacusAI.generateAdCopyVariations(
            analysis,
            constitution,
            influencer?.name
        )
        ...
        const { error: updateError } = await adminClient
            .from('projects')
            .update({ ad_copies: adCopies })
            .eq('id', projectId)

        if (updateError) {
            console.error('Failed to save ad copies:', updateError)
            return NextResponse.json({ error: 'Failed to save ad copies' }, { status: 500 })
        }

        return NextResponse.json({ success: true, data: adCopies })
    } catch (error) {
